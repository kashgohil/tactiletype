...
| Variable | Required | Default / example | Purpose |
|----------|----------|-------------------|---------|
| `VITE_API_URL` | Recommended | `http://localhost:3021` | REST API base URL |
| `VITE_WS_URL` | Recommended | `ws://localhost:3021/ws` | WebSocket URL |

Minimal working `apps/web/.env`:

```env
VITE_API_URL=http://localhost:3021
VITE_WS_URL=ws://localhost:3021/ws
```
...
| Service | Port | URL |
|---------|------|-----|
| API (HTTP + WS) | 3021 | `http://localhost:3021`, `ws://localhost:3021/ws` |
| Web (Vite) | 3020 | `http://localhost:3020` |

Vite proxies `/api` → `http://localhost:3021` (see `apps/web/vite.config.ts`). Still set `VITE_API_URL` / `VITE_WS_URL` so client code does not fall back to inconsistent defaults.

**If 3021 or 3020 is already in use** (other projects), either free the port or change `PORT` / Vite `server.port` **and** update `FRONTEND_URL`, `BASE_URL`, `VITE_API_URL`, and `VITE_WS_URL` to match.

The two ports are adjacent on purpose, but that means Vite's "port taken, trying the next one" behaviour would put the web app straight onto the API's port. If 3020 is occupied, set an explicit `server.port` rather than letting Vite pick.
...
bun run dev:api    # http://localhost:3021
bun run dev:web    # http://localhost:3020
```
...
```bash
# API health
curl -sS http://localhost:3021/api
# → JSON with message "tactiletype API Server", status "healthy"

# Sample texts (after seed)
curl -sS http://localhost:3021/api/tests/texts | head
...
WebSocket (once multiplayer is exercised): `ws://localhost:3021/ws`.
...
[ ] bun run db:migrate
[ ] bun run db:seed
[ ] Ensure ports 3021 and 3020 are free (or remapped consistently)
[ ] bun run dev
[ ] curl http://localhost:3021/api  → healthy JSON
[ ] open http://localhost:3020
```
