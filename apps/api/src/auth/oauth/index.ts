import { BaseOAuthProvider } from './base';
import { GitHubOAuthProvider } from './github';
import { GoogleOAuthProvider } from './google';

export { BaseOAuthProvider, GitHubOAuthProvider, GoogleOAuthProvider };

export class OAuthProviderFactory {
  private static providers: Map<string, BaseOAuthProvider> = new Map();

  static registerProvider(name: string, provider: BaseOAuthProvider) {
    OAuthProviderFactory.providers.set(name, provider);
  }

  static getProvider(name: string): BaseOAuthProvider | undefined {
    return OAuthProviderFactory.providers.get(name);
  }

  static getAllProviders(): string[] {
    return Array.from(OAuthProviderFactory.providers.keys());
  }

  static initialize() {
    // Initialize providers from environment variables
    const googleClientId = process.env.GOOGLE_CLIENT_ID;
    const googleClientSecret = process.env.GOOGLE_CLIENT_SECRET;
    const githubClientId = process.env.GITHUB_CLIENT_ID;
    const githubClientSecret = process.env.GITHUB_CLIENT_SECRET;
    const baseUrl = process.env.BASE_URL || 'http://localhost:3021';

    if (googleClientId && googleClientSecret) {
      const googleProvider = new GoogleOAuthProvider(
        googleClientId,
        googleClientSecret,
        `${baseUrl}/api/auth/sso/google/callback`
      );
      OAuthProviderFactory.registerProvider('google', googleProvider);
    }

    if (githubClientId && githubClientSecret) {
      const githubProvider = new GitHubOAuthProvider(
        githubClientId,
        githubClientSecret,
        `${baseUrl}/api/auth/sso/github/callback`
      );
      OAuthProviderFactory.registerProvider('github', githubProvider);
    }
  }
}
