...
export interface WebSocketEventHandlers {
  ...
  onSpectatorJoined?: (data: { spectator: { userId: string; username: string } }) => void;
...
export class WebSocketService {
        ...
        this.ws.onmessage = (event) => {
          try {
            ...
            if (message.type === 'error' && !this.isAuthenticated && !settled) {
              settleErr(new Error(message.data?.error || 'Authentication failed'));
            }
          } catch (error) {
  ...
  // Room management
  joinRoom(roomId: string, userId: string, username: string, spectate = false): void {
