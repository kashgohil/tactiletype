...
export const RaceChat: React.FC<RaceChatProps> = ({ messages, onSend, disabled }) => {
  ...
  // Scroll the log, not the page. `scrollIntoView` walks every scrollable
  // ancestor, so pinning a sentinel to the bottom of the chat also scrolled
  // the document down to it - which on this layout meant the footer slid into
  // view on mount and again after every message sent.
  // biome-ignore lint/correctness/useExhaustiveDependencies: the message count is the trigger, not a read
  useEffect(() => {
  ...
  return (
    <section className="rounded-2xl border border-accent/15 bg-accent/[0.05] flex flex-col h-64 md:h-80">
      ...
      <div ref={logRef} className="flex-1 overflow-y-auto px-3 py-2 space-y-2 text-sm">
        {messages.length === 0 && (
