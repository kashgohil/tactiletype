...
function PrimaryMetric({
  ...
  return (
    <div className={cn(panelSurface, 'p-5 md:p-6 flex flex-col gap-2 min-h-[120px]')}>
...
function SecondaryMetric({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="px-4 py-3 text-center">
      <div className="text-lg font-semibold text-text font-mono tabular-nums">{value}</div>
      <div className="text-xs text-text/50 mt-0.5">{label}</div>
    </div>
  );
}

export const MetricHierarchy: React.FC<MetricHierarchyProps> = ({ stats, isLoading }) => {
  if (isLoading) {
    return (
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <SecondaryMetric label="Avg WPM" value={Math.round(Number(stats.avgWpm))} />
        <SecondaryMetric label="Longest streak" value={stats.longestStreak} />
        <SecondaryMetric label="Time practiced" value={formatTime(stats.totalTime)} />
      </div>
    </div>
  );
};
