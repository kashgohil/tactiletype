...
import { FONT_SIZE_CLASS, useTestPreferences } from '@/hooks/useTestPreferences';
import { cn } from '@/lib/utils';
import { type CodeToken, codeTokenClass } from '@/utils/codeHighlight';
...
export const TypingSurface: React.FC<TypingSurfaceProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    const boxFor = (index: number): CaretBox | null => {
      const el = container.querySelector(`[data-char-index="${index}"]`) as HTMLElement | null;
      if (!el) return null;
  ...
  const caretShape = (box: CaretBox, override?: string) => {
    ...
    switch (style) {
      ...
      case 'underline':
        ...
        break;
      case 'box':
        className += cn('border-2 rounded-sm', override ?? 'border-accent', pulse);
        size = { width: box.w, height: box.h };
        break;
      default:
  ...
  return (
    ...
    <div
      ...
      ref={surfaceRef}
      // A non-interactive surface is a read-only rendering of the text, but it
      // still needs a role that carries `aria-label`.
      role={interactive ? 'textbox' : 'group'}
    ...
    >
      ...
      {words()}
      {trailingAnchor && (
        <div data-char-index={text.length} aria-hidden className="opacity-0 select-none">
          {' '}
        </div>
      )}
      {renderMarkers()}
